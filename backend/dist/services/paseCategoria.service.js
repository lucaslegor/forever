...
exports.ejecutarPaseCategoriaFutbolMasculino = ejecutarPaseCategoriaFutbolMasculino;
 ...
 */
async function ejecutarPaseCategoriaFutbolMasculino() {
    const detalle = [];
    const errores = [];
    const disciplina = await prisma_1.default.disciplina.findFirst({
        where: { nombre: { in: ['Futbol', 'Fútbol'] } },
    });
    if (!disciplina) {
        throw new Error('Disciplina Fútbol no encontrada');
    }
    const genero = await prisma_1.default.genero.findFirst({
    ...
    const [infantiles, juveniles, mayores] = await Promise.all([
        prisma_1.default.categoria.findFirst({ where: { nombre: 'Infantiles' } }),
        prisma_1.default.categoria.findFirst({ where: { nombre: 'Juveniles' } }),
        prisma_1.default.categoria.findFirst({ where: { nombre: 'Mayores' } }),
    ]);
    ...
    const mapSubcat = (categoriaId, nombre) => subcategorias.find((s) => s.categoriaId === categoriaId && s.nombre === nombre);
    ...
    for (const d of deportistas) {
        ...
        let nuevaCategoriaId;
        let nuevaSubcategoriaId = null;
        ...
        await prisma_1.default.deportista.update({
            where: { id: d.id },
            data: {
...
//# sourceMappingURL=paseCategoria.service.js.map
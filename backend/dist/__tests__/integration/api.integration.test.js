...
describe('Tests de API con datos del Seed', () => {
    ...
    describe('Auth - Login', () => {
        it('deberia hacer login como admin', async () => {
            ...
            expect(response.body.data.user.rol).toBe(client_1.Rol.ADMIN);
    ...
    describe('Disciplinas', () => {
        it('deberia listar disciplinas (cualquier usuario autenticado)', async () => {
            ...
            const nombres = response.body.data.map((d) => d.nombre);
    ...
    describe('Deportistas', () => {
        it('admin deberia listar deportistas', async () => {
            ...
            const dnis = response.body.data.data.map((d) => d.dni);
    ...
    describe('Cuotas', () => {
        ...
        it('admin deberia poder asignar cuota o devolver 409 si ya existe', async () => {
            ...
            const depResponse = await (0, supertest_1.default)(app_1.default)
                .get('/api/deportistas')
                .set('Authorization', `Bearer ${adminToken}`);
            const deportista = depResponse.body.data.data.find((d) => d.dni === '41234567'); // María López
    ...
    describe('Grupos Familiares', () => {
        it('admin deberia listar grupos familiares', async () => {
            ...
            const familiaPerez = response.body.data.find((g) => g.nombre === 'Familia Pérez');
